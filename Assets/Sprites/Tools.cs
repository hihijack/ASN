...
//	}
	...
	public static tk2dTextMesh GetTextMeshInChildByPath(GameObject gobjParent, string path){
		tk2dTextMesh r = null;
		if (gobjParent == null){
			return r;
		}
		...
		if (tf != null){
			r = tf.gameObject.GetComponent<tk2dTextMesh>();
		}
		return r;
	}
	
	public static tk2dSprite GetSpriteInChildByPath(GameObject gobjParent, string path){
		tk2dSprite r = null;
		if (gobjParent == null){
			return r;
		}
		Transform tf = GetTransformInChildByPath(gobjParent, path);
		if (tf != null){
			r = tf.gameObject.GetComponent<tk2dSprite>();
		}
		return r;
	}
...
//		Transform tf = GetTransformInChildByPath(gobjParent, path);
	...
	/// <summary>
	/// Determines whether gobjA is in the right of gobjB.
	/// </summary>
	/// <returns>
	/// <c>true</c> if this instance is in the right the specified gobjA gobjB; otherwise, <c>false</c>.
	/// </returns>
	/// <param name='gobjA'>
	/// If set to <c>true</c> gobj a.
	/// </param>
	/// <param name='gobjB'>
	/// If set to <c>true</c> gobj b.
	/// </param>
	public static bool IsInTheRight(GameObject gobjA, GameObject gobjB){
		bool r = false;
		if(gobjA.transform.position.x > gobjB.transform.position.x){
			r = true;
		}
		return r;
	}
